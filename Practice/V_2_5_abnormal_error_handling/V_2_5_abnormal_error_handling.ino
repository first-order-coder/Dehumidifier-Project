...
// Tank-1 floats (external pull-up → switch to GND)
const int PIN_T1_HIGH   = 2;    // sensor 2 (upper)
const int PIN_T1_LOW    = 3;    // sensor 1 (lower)

// Actuators
const int PIN_PUMP      = 8;    // pump relay IN/driver
const int PIN_HUMID     = 9;    // humidifier relay IN/driver (energized = humidifier OFF)
const int PIN_VALVE4    = 5;    // main discharge valve relay (normally closed; energize to open)

// Indicators
const int PIN_FAULT_LED  = 6;                 // fault LED
const int PIN_MOTOR_LED  = LED_BUILTIN;       // D13 onboard LED
const int PIN_VALVE4_LED = 7;                 // valve-4 status LED (set -1 to disable)

// Optional button(s)
const bool ENABLE_CLEAR_BTN = false;
const int  PIN_CLEAR_BTN    = 4;              // if enabled: ext pull-up, button to GND
const int  PIN_MANUAL_BTN   = 10;             // internal pull-up; button to GND
...
#if HAS_TANK2
  const int PIN_VALVE5    = 11;   // bypass valve to Tank-2 (energize=open) -> fill T2
  const int PIN_VALVE8    = 12;   // switching valve -> drain T2
  const bool RELAY_VALVE5_ACTIVE_LOW = false;
  const bool RELAY_VALVE8_ACTIVE_LOW = false;

  // Tank-2 sensors (use analog pins as digital inputs with pull-ups)
  const int PIN_T2_LOW    = A1;   // sensor 6 (LOW=water present)
  const int PIN_T2_HIGH   = A2;   // sensor 7 (HIGH=full)
#endif

// Defrost spiral (only used in v11/v21)
#if HAS_DEFROST
  const int PIN_DEFROST   = A3;   // heater driver (energize to heat)
  const bool DEFROST_ACTIVE_LOW = false;
#endif

// ADC for shunt (pump current proxy)
const int   SHUNT_ADC_PIN        = A0;     // sense on pump line (mV)
const bool  USE_INTERNAL_ADC_REF = true;   // 1.1V ref
...
const uint8_t  AVG_WINDOW          = 5;       // 5-second rolling average
...
/*************** NEW: Drain-time guards (Tank-1) ***************/
const uint32_t T_UPPER_FALL_MAX_MS  = 120UL * 1000UL;        // 2 minutes for upper sensor to go LOW->HIGH
const uint32_t T_TOTAL_DRAIN_MAX_MS = 10UL  * 60UL * 1000UL; // 10 minutes for lower to go LOW->HIGH
...
uint32_t readShunt_mV() {
  ...
  if (!refInit) {
    ...
    delay(5); (void)analogRead(SHUNT_ADC_PIN); refInit=true;
  }
...
/*************** Placeholder SMS ***************/
inline void sendSMS(const __FlashStringHelper* msg){ SER_PRINT(F("[SMS] ")); SER_PRINTLN(msg); }
inline void sendSMS(const char* msg){ SER_PRINT(F("[SMS] ")); SER_PRINTLN(msg); }

/*************** NEW: Tank-1 drain-time guard ***************/
enum DrainResult {
  DRAIN_IN_PROGRESS,
  DRAIN_OK,
  FAULT_FLOW_OR_UPPER_STUCK,     // upper never dropped
  FAULT_UPPER_SENSOR_CONTRADICT, // lower empty while upper still high
  FAULT_LOWER_SENSOR_OR_SLOW,    // upper dropped, lower never did
};

struct DrainCycle {
  bool active = false;            // guard armed
  bool enable = false;            // allow arming for next ON (only normal Tank-1 cycle)
  uint32_t tStart = 0;

  // initial sensor states
  bool upperAtStart = false;      // true if water at upper (t1High == LOW)
  bool lowerAtStart = false;      // true if water at lower (t1Low  == LOW)

  // transitions observed
  bool sawUpperDrop = false;      // detected upper LOW->HIGH
  bool sawLowerDrop = false;      // detected lower LOW->HIGH
} drain;

void startTank1DrainGuard() {
  // Called only for NORMAL Tank-1 automatic start
  drain.enable = true;  // allow arming on next pump ON
}

void armDrainGuardIfEligibleOnPumpOn() {
  if (!drain.enable) return;            // not a normal Tank-1 run
  drain.enable = false;                 // consume
  drain.active = true;
  drain.tStart = millis();
  drain.sawUpperDrop = false;
  drain.sawLowerDrop = false;

  // sample initial states (debounced)
  int u = t1HighF.read(FLOAT_FILTER_MS);
  int l = t1LowF.read(FLOAT_FILTER_MS);
  drain.upperAtStart = (u == LOW);     // water touching upper
  drain.lowerAtStart = (l == LOW);     // water present at lower

  SER_PRINTLN(F("[Guard] Tank-1 drain guard ARMED."));
}

DrainResult updateTank1DrainGuard() {
  if (!drain.active) return DRAIN_OK;

  const uint32_t now = millis();
  const uint32_t elapsed = now - drain.tStart;

  const int upperNowPin = t1HighF.read(FLOAT_FILTER_MS);
  const int lowerNowPin = t1LowF.read(FLOAT_FILTER_MS);

  const bool upperNowWater = (upperNowPin == LOW); // LOW = water present
  const bool lowerNowWater = (lowerNowPin == LOW);

  // track transitions: water -> no water (LOW -> HIGH)
  if (drain.upperAtStart && !upperNowWater) drain.sawUpperDrop = true;
  if (drain.lowerAtStart && !lowerNowWater) drain.sawLowerDrop = true;

  // Contradiction: lower became empty while upper stayed wet all the time
  if (!lowerNowWater && upperNowWater && !drain.sawUpperDrop) {
    drain.active = false;
    return FAULT_UPPER_SENSOR_CONTRADICT;
  }

  // Upper must drop within limit
  if (elapsed >= T_UPPER_FALL_MAX_MS && !drain.sawUpperDrop) {
    drain.active = false;
    return FAULT_FLOW_OR_UPPER_STUCK;
  }

  // Total drain deadline
  if (elapsed >= T_TOTAL_DRAIN_MAX_MS) {
    if (drain.sawUpperDrop && !drain.sawLowerDrop) {
      drain.active = false;
      return FAULT_LOWER_SENSOR_OR_SLOW;
    }
  }

  // Normal completion: both sensors report "no water"
  if (!upperNowWater && !lowerNowWater) {
    drain.active = false;
    return DRAIN_OK;
  }
...
void cancelTank1DrainGuard() {
  if (drain.active) SER_PRINTLN(F("[Guard] Tank-1 drain guard DISARMED."));
  drain.active = false;
  drain.enable = false;
}
...
void applyPumpState(bool on){
  ...
  if (on){
    ...
    // NEW: if a normal Tank-1 cycle requested this ON, arm the guard
    armDrainGuardIfEligibleOnPumpOn();
  } else {
    if (pumpRunning){
      uint32_t now = millis();
...
#if HAS_TANK2
    // At the moment of stopping, decide whether Tank-2 needs draining.
    // T2_LOW == LOW means "water present".
    ...
    // Post-stop main line flush (only if not suppressing)
    if (!suppressNextMainFlush) {
      ...
      // consume the one-time suppression flag
      suppressNextMainFlush = false;
    }

    // When pump stops (by any reason), disarm guard
    cancelTank1DrainGuard();
  }
  pumpRunning = on;
  lastSwitchMs = millis();
}
...
// (kept for reference; not used in the new frozen path)
bool tryRouteToTank2() {
#if HAS_TANK2
  SER_PRINTLN(F("Frozen detected -> attempting route to Tank-2 (valve5+8)."));
  valve4_set(false);
  valve5_set(true);
  delay(50);
  valve8_set(true);
  ...
  uint32_t safety = millis() + 5UL*60UL*1000UL; // 5 min hard cap
  while (millis() < safety) {
    if (flowAvgActive && millis() >= flowStartAfterMs && (millis()-lastSampleMs)>=SAMPLE_MS){
      ...
      if (shuntCount>=AVG_WINDOW){
        ...
        if (avg_mV >= FLOW_BAD_mV){
          SER_PRINTLN(F("Reroute still abnormal -> routing failed."));
          applyPumpState(false);
          valve8_set(false); valve5_set(false);
          valve4_openTimed(5000);
          return false;
        }
      }
    }
    int t1low = digitalRead(PIN_T1_LOW);
    if (t1low == HIGH){
      pendingStopReason='R'; applyPumpState(false);
      valve8_set(false); valve5_set(false);
      valve4_openTimed(5000);
      SER_PRINTLN(F("Routing succeeded: Tank-1 emptied to Tank-2."));
      return true;
    }
    delay(5);
  }
  SER_PRINTLN(F("Routing timeout -> failed."));
  applyPumpState(false);
  valve8_set(false); valve5_set(false);
  valve4_openTimed(5000);
  return false;
...
// Optional fallback defrost (kept in case you want it later)
bool tryDefrostAndRetry(bool t2High){
#if HAS_DEFROST
  if (!policyAllowsDefrostNow(t2High)) return false;
  SER_PRINT(F("Frozen detected -> DEFROST for ")); SER_PRINT(DEFROST_MS/60000UL); SER_PRINTLN(F(" min."));
  defrost_set(true);
  uint32_t until = millis()+DEFROST_MS;
  while ((long)(millis()-until) < 0) delay(50);
  defrost_set(false);
  ...
  uint32_t endTry = millis()+20000UL;
  while ((long)(millis()-endTry) < 0){
    if (flowAvgActive && millis()>=flowStartAfterMs && (millis()-lastSampleMs)>=SAMPLE_MS){
      lastSampleMs+=SAMPLE_MS;
      uint16_t mv=(uint16_t)readShunt_mV();
      shuntBuf[shuntHead]=mv; shuntHead=(shuntHead+1)%AVG_WINDOW; if(shuntCount<AVG_WINDOW) shuntCount++;
      if (shuntCount>=AVG_WINDOW){
        ...
        float avg_mV=sum/(float)AVG_WINDOW;
        if (avg_mV >= FLOW_BAD_mV){
          SER_PRINTLN(F("After defrost: still abnormal."));
          applyPumpState(false);
          valve4_openTimed(5000);
          return false;
        }
    ...
    if (digitalRead(PIN_T1_LOW)==HIGH){
      pendingStopReason='D'; applyPumpState(false);
      valve4_openTimed(5000);
      SER_PRINTLN(F("After defrost: Tank-1 emptied normally."));
      return true;
    }
    delay(5);
  }
...
bool routeFrozenToTank2_UntilT1Empty(uint32_t timeout_ms = 5UL*60UL*1000UL) {
  ...
  // Ensure main is closed; prepare to FILL Tank-2
  valve4_set(false);
  valve5_set(true);        // open bypass to Tank-2 (fill)
  // IMPORTANT: Valve-8 remains OFF here

  // Start pump now (bypass dwell)
  lastSwitchMs = millis() - MIN_DWELL_MS;
  // Do NOT arm Tank-1 guard for this special run:
  drain.enable = false;
  applyPumpState(true);
  ...
  // Run until T1 low becomes HIGH (empty) or timeout
  uint32_t cap = millis() + timeout_ms;
  bool emptied = false;
  while (millis() < cap) {
    if (t1LowF.read(FLOAT_FILTER_MS) == HIGH) { emptied = true; break; }
    delay(10);
  }

  // Stop pump, close Valve-5, then short main flush
  pendingStopReason = 'F';  // 'F' = frozen reroute
  applyPumpState(false);
  valve5_set(false);        // close bypass
...
void startManualDrain(){
  ...
  // Do NOT arm Tank-1 guard for manual mode
  drain.enable = false;
  cancelTank1DrainGuard();
...
void setup(){
  ...
  // Inputs
  t1HighF.begin(PIN_T1_HIGH);  // external pull-ups on floats
  t1LowF.begin(PIN_T1_LOW);
  ...
  EEPROM.get(EE_ADDR_BOOT_COUNT, bootCount); bootCount++; EEPROM.put(EE_ADDR_BOOT_COUNT, bootCount);
  ...
  // Startup drain only if water present in Tank-1 (low = LOW)
  ...
  if (!faultLatched && t1LowF.read(FLOAT_FILTER_MS) == LOW){
    ...
    // Arm guard for this normal Tank-1 cycle
    startTank1DrainGuard();
    lastSwitchMs = millis() - MIN_DWELL_MS;
    applyPumpState(true);
  } else {
...
void loop(){
  ...
  if (SER_AVAILABLE()){
    ...
    switch(c){
      ...
      case 'Z': bootCount=0; EEPROM.put(EE_ADDR_BOOT_COUNT, bootCount); SER_PRINTLN(F("Boot counter reset.")); break;
  ...
  if (faultLatched){
    ...
    // Humidifier safety: Tank-2 full => disable humidifier
#if HAS_TANK2
    ...
    if (manualDrainMode){
      ...
      if (!pumpRunning){
        if (t1High==LOW){
          // Arm guard for NORMAL Tank-1 start
          startTank1DrainGuard();
          applyPumpState(true);
          SER_PRINTLN(F("Reason: T1 HIGH -> start pump (guard armed)."));
        }
      } else {
    ...
    if (pumpRunning && flowAvgActive){
      ...
      if (now>=flowStartAfterMs && (now-lastSampleMs)>=SAMPLE_MS){
        ...
        if (shuntCount>=AVG_WINDOW){
          ...
          if (avg_mV >= FLOW_BAD_mV){
            // Frozen case: FILL Tank-2 only (Valve-5). Valve-8 remains OFF here.
            SER_PRINTLN(F("Frozen/abnormal flow detected by rolling average."));
            // Not a normal Tank-1 cycle → ensure guard won't arm here
            drain.enable = false;
...
#if HAS_TANK2
    // Post-run Tank-2 drain only when we *know* T2 has water (t2DrainPending set earlier)
    if (!faultLatched && !pumpRunning && t2DrainPending) {
      SER_PRINTLN(F("Executing scheduled Tank-2 drain..."));
      valve4_set(false);       // close main line
      // Turn Valve-8 ON only now (we're draining Tank-2)
      valve8_set(true);
      delay(50);

      // Start pump now (bypass dwell)
      lastSwitchMs = millis() - MIN_DWELL_MS;
      // This is not a normal Tank-1 cycle → do not arm guard
      drain.enable = false;
      applyPumpState(true);

      // Run until T2 low becomes HIGH (empty) or timeout
      ...
      pendingStopReason = 't';
      applyPumpState(false);
      valve8_set(false);             // stop draining path
      valve4_openTimed(5000);        // short main flush *after* T2 drain
      SER_PRINTLN(F("Tank-2 drain complete. Post-drain main flush done."));
      t2DrainPending = false;        // clear request
    }
#endif
  }

  // --- NEW: Drain guard evaluation (only when not faulted) ---
  if (!faultLatched && drain.active){
    DrainResult r = updateTank1DrainGuard();
    if (r != DRAIN_IN_PROGRESS){
      switch(r){
        case DRAIN_OK:
          sendSMS(F("OK: Tank-1 drained successfully.")); 
          break;
        case FAULT_FLOW_OR_UPPER_STUCK:
          tripFaultInstant("FAULT: No upper level drop in time. Possible no flow/blocked line. System stopped.");
          break;
        case FAULT_UPPER_SENSOR_CONTRADICT:
          tripFaultInstant("FAULT: Upper sensor stuck HIGH (lower emptied first). System stopped.");
          break;
        case FAULT_LOWER_SENSOR_OR_SLOW:
          // softer reaction: stop pump, optional humidifier policy
          pendingStopReason='L';
          applyPumpState(false);
          humidOff(); // optional: keep OFF to be conservative
          sendSMS(F("ALERT: Lower sensor didn’t go empty by deadline. Likely lower sensor fault or slow drain. Pump stopped."));
          break;
        default: break;
      }
      // guard is disarmed inside update on terminal states
    }
  }
