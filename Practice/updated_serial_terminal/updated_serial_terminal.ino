...
void setup() {
  ...
  SER_PRINT(F("Lifetime pump ON time (EEPROM): "));
  SER_PRINT(totalRunSec); SER_PRINTLN(F(" s"));
...
void loop() {
  ...
  if (SER_AVAILABLE()){
    char c = SER_READ();
    switch (c) {
      case 'C': faultLatched=false; saveFaultToEEPROM(false); setFaultLED(false); SER_PRINTLN(F("Fault CLEARED via Serial.")); break;
      case 'R': {
