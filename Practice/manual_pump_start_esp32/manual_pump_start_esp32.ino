...
struct Debounce {
  ...
  void begin(int p){
    pin=p;
    // Using INPUT_PULLUP so you can wire switch to GND without external parts
...
void stopManualDrain(char reasonCode){
  ...
  printStamp(); Serial.print(F("Manual DRAIN: stop (reason="));
  Serial.print(reasonCode); Serial.println(F(")."));
}
...
void setup() {
  ...
  // Inputs/outputs (floats + manual button use internal pull-up)
  topF.begin(FLOAT_TOP_PIN);
  botF.begin(FLOAT_BOTTOM_PIN);
  manBtn.begin(MANUAL_BTN_PIN);        // <-- internal pull-up, wire to GND
  ...
  printlnTS(F("Serial cmds: 'C' clear fault, 'R' report, 'H' run history, 'X' clear run history, 'B' boot info, 'Z' reset boot count, 'S' session history, 't' show RTC, 'T=YYYY-MM-DD HH:MM:SS' set RTC, 'M' manual drain"));
...
void loop() {
  ...
  if (faultLatched) {
    ...
    valveClose();
    valveOpenUntilMs = 0;
    // cancel manual mode on fault
    manualDrainMode = false;
  } else {
    // ===== Manual drain overrides normal control =====
    if (manualDrainMode) {
      // keep pump ON (bypass dwell already handled)
      if (!pumpRunning) {
        lastSwitchMs = millis() - MIN_DWELL_MS;
        applyPumpState(true);
      }
      // stop when empty or timeout
      if (bottom == HIGH) {
      ...
      if (now >= flowWindowStartMs) {
        ...
        if (now - flowWindowStartMs >= FLOW_VERIFY_MS) {
          ...
          if (avg_mV >= FLOW_BAD_mV) {
            ...
            manualDrainMode = false; // cancel manual on fault
          } else {
