...
/*************** ESP32-S3 PIN MAP (your list) ***************
 ...
 14  IN_DB calibration switch   (used here as MANUAL button)
 ...
 46  IN_DB empty tank (unused here)
************************************************************/

/*************** PINS (remapped to ESP32-S3) ***************/
// Tank-1 floats
const int PIN_T1_HIGH   = 7;    // top_sensor (upper)  LOW=water present
const int PIN_T1_LOW    = 33;   // bottom_sensor (lower) LOW=water present
...
// Indicators
const int PIN_FAULT_LED  = 41;                 // heartbeat LED reused as fault
const int PIN_MOTOR_LED  = 41;                 // reuse same LED for pump
const int PIN_VALVE4_LED = -1;                 // no dedicated LED

// Optional button(s)
const bool ENABLE_CLEAR_BTN = false;
const int  PIN_CLEAR_BTN    = 4;               // reserved (not used)
const int  PIN_MANUAL_BTN   = 14;              // use as Manual Drain button
...
// Tank-2 hardware (only used in v20/v21)
  ...
  // Tank-2 sensors
  const int PIN_T2_LOW    = 20;   // LOW=water present
  const int PIN_T2_HIGH   = 10;   // HIGH=full
#endif
...
const bool  USE_INTERNAL_ADC_REF = false;  // ESP32 ignores this
...
const uint32_t T_UPPER_FALL_MAX_MS  = 120UL * 1000UL;        // 2 minutes for upper to go LOW->HIGH
...
uint32_t readShunt_mV() {
  ...
  // set attenuation once for full ~3.3V range if needed
  static bool attn_set=false; if(!attn_set){ analogSetPinAttenuation(SHUNT_ADC_PIN, ADC_11db); attn_set=true; }
#endif
...
/*************** SMS stub (replace with SIM808/SIM7600) ***************/
...
enum DrainResult {
  ...
  FAULT_UPPER_SENSOR_CONTRADICT, // lower empty while upper stayed high
  FAULT_LOWER_SENSOR_OR_SLOW,    // upper dropped, lower never did
};

struct DrainCycle {
  bool active = false;            // guard armed
  bool enable = false;            // allow arming on next normal T1 ON
  uint32_t tStart = 0;

  bool upperAtStart = false;      // T1 upper had water at start
  bool lowerAtStart = false;      // T1 lower had water at start

  bool sawUpperDrop = false;      // saw upper LOW->HIGH
  bool sawLowerDrop = false;      // saw lower LOW->HIGH
} drain;

void startTank1DrainGuard() { drain.enable = true; }

void cancelTank1DrainGuard() {
  if (drain.active) SER_PRINTLN(F("[Guard] Tank-1 drain guard DISARMED."));
  drain.active = false; drain.enable = false;
}

void armDrainGuardIfEligibleOnPumpOn() {
  if (!drain.enable) return;
  drain.enable = false;
  drain.active = true;
  drain.tStart = millis();
  ...
  drain.upperAtStart = (u == LOW);
  drain.lowerAtStart = (l == LOW);
...
DrainResult updateTank1DrainGuard() {
  ...
  // Normal completion: both sensors "no water"
...
void applyPumpState(bool on){
  ...
  if (on){
    ...
    // Arm drain guard if a NORMAL Tank-1 cycle requested this ON
    ...
    // Whenever pump stops, disarm guard
...
bool routeFrozenToTank2_UntilT1Empty(uint32_t timeout_ms = 5UL*60UL*1000UL) {
  ...
  // This is NOT a normal Tank-1 run -> don't arm the guard
  drain.enable = false;
  cancelTank1DrainGuard();
  applyPumpState(true);
...
void setup(){
  ...
  // Inputs (most “IN_J/IN_DB” have external resistors -> still safe to enable pullups)
  ...
  // Startup drain only if water present in Tank-1 (low sensor = LOW)
...
void loop(){
  ...
  if (faultLatched){
      ...
      if (now>=flowStartAfterMs && (now-lastSampleMs)>=SAMPLE_MS){
        ...
        if (shuntCount>=AVG_WINDOW){
          ...
          if (avg_mV >= FLOW_BAD_mV){
            ...
            // Not a normal Tank-1 cycle → ensure guard won't arm
            drain.enable = false;
            cancelTank1DrainGuard();
            (void)routeFrozenToTank2_UntilT1Empty(); // do not emergency-trip
            flowAvgActive = false; // let normal logic continue next loop
    ...
    if (!faultLatched && !pumpRunning && t2DrainPending) {
      ...
      lastSwitchMs = millis() - MIN_DWELL_MS;
      // Not a normal Tank-1 cycle -> don't arm guard
      drain.enable = false;
      cancelTank1DrainGuard();
      applyPumpState(true);
