...
/* ======= FAULT LED ======= */
const int  FAULT_LED_PIN          = 6;     // choose any free digital pin
...
void setup() {
  ...
  // ======= POWER-UP: clear any stored fault (as requested) =======
  ...
  Serial.println(F("To CLEAR fault during runtime: send 'C' over Serial or press the CLEAR button (if enabled)."));

  // === Startup drain ONLY if bottom float says "water present" ===
  // LOW = float UP (water present) with your wiring.
...
void loop() {
  ...
  if (faultLatched) {
      ...
      if (millis() - pumpOnAtMs >= FAULT_BLANK_MS) {
        ...
        if (mv > SHORT_mV) {
          ...
          if (millis() - overStart >= SHORT_MS) {
            ...
            saveFaultToEEPROM(true);      // store (will be cleared next power-up)
    ...
    // Keep LED mirrored to current fault state (belt & suspenders)
